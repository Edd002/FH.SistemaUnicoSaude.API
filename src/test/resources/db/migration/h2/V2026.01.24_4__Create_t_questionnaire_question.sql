...
ALTER TABLE public.t_questionnaire_question ADD FK_QUESTIONNAIRE_UK_FIELD INT8 AS (CASE deleted WHEN TRUE THEN NULL ELSE fk_questionnaire END);
ALTER TABLE public.t_questionnaire_question ADD FK_QUESTION_UK_FIELD INT8 AS (CASE deleted WHEN TRUE THEN NULL ELSE fk_question END);

CREATE UNIQUE INDEX T_QUESTIONNAIRE_QUESTION__HASH_ID_UK ON public.t_questionnaire_question (hash_id);
CREATE UNIQUE INDEX T_QUESTIONNAIRE_QUESTION__FK_QUESTIONNAIRE_UK ON public.t_questionnaire_question (FK_QUESTIONNAIRE_UK_FIELD);
CREATE UNIQUE INDEX T_QUESTIONNAIRE_QUESTION__FK_QUESTION_UK ON public.t_questionnaire_question (FK_QUESTION_UK_FIELD);